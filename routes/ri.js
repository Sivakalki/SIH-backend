    ...
    try {
        ...
        if (!ri) {
            return res.status(500).json({ 
                success: false,
                message: "ri role not found in system" 
            });
        }
...
router.post("/recheck/:app_id", async (req, res) => {
    try {
        ...
        const application = await prisma.application.findUnique({
            ...
            select: { current_stage_id: true, ri_user_id: true }
        });
        ...
        if (user.role_id !== 3 || application.ri_user_id || application.current_stage_id !== 3) {
...
router.post("/create_report/:app_id", async (req, res) => {
    try {
        ...
        const app_user = await prisma.rI.findFirst({
            where: {
                ri_id: app_ri.ri_user_id
            },
            select: {
                user_id: true
            }
        })
        if (handler.user_id !== app_user.user_id) {
